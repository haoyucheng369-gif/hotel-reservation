...
namespace HotelReservation.UseCases.Tests.Reservations.CancelReservation;
...
public class CancelReservationCommandHandlerTests
{
    ...
    public async Task Handle_ShouldFail_WhenReservationDoesNotExist()
    {
        ...
        var handler = new CancelReservationCommandHandler(repository);
        var command = new CancelReservationCommand(new ReservationId(Guid.NewGuid()));

        var result = await handler.Handle(command, CancellationToken.None);

        Assert.False(result.IsSuccess);
        Assert.Equal(CancelReservationErrors.ReservationNotFound, result.Error);
        Assert.False(repository.SaveChangesWasCalled);
    }

    [Fact]
    public async Task Handle_ShouldFail_WhenReservationIsAlreadyCancelled()
    {
        ...
        Assert.False(result.IsSuccess);
        Assert.Equal(CancelReservationErrors.ReservationAlreadyCancelled, result.Error);
        Assert.False(repository.SaveChangesWasCalled);
    }
    ...
    [Fact]
    public async Task Handle_ShouldSucceed_WhenReservationIsConfirmed()
    {
        var reservation = CreateReservation();
        reservation.Confirm();
        var repository = new FakeReservationRepository(reservation);
